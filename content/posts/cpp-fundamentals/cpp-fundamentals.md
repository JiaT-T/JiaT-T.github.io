...
### 3).C++和C分别使用什么函数来做内存的分配和释放？有什么区别，能否混用？
...
### 3).weak_ptr是如何解决shared_ptr的循环引用问题的：
...
### 4).既然weak_ptr是为了解决shared_ptr的循环引用，那为什么不用raw ptr来解决这个问题：
